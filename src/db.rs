...
mod tests {
	...
	fn test_indexed_btree_3() {
		...
		// test_indexed_btree_inner_case(EnableCommitPipelineStages::Standard);
	}
	fn test_indexed_btree_inner_case(db_test: EnableCommitPipelineStages) {
	...
	fn test_indexed_btree_inner_3(db_test: EnableCommitPipelineStages) {
		...
		for _ in 0..100 {
			...
			iter.seek(&dbg!(at).to_be_bytes()).unwrap();

			let at = if rand::random() {
				let take = dbg!(rand::thread_rng().gen_range(0..100));
				let got = std::iter::from_fn(|| iter.next().unwrap())
					.map(|(k, _)| u64::from_be_bytes(k.try_into().unwrap()))
					.take(take)
					.collect::<Vec<_>>();
